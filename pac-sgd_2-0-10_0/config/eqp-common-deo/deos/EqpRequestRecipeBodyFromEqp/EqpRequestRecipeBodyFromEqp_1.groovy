...
''')
class EqpRequestRecipeBodyFromEqp_1
{
    ...
    public byte[] getRecipeBody()
    {
        ...
        def replyData = reply.getData().getPPBODY()
        if (replyData instanceof SecsAsciiItem)
        {
            SecsAsciiItem ppbody = (SecsAsciiItem) replyData            
            return ppbody.getString().getBytes()
        }
        else if (replyData instanceof SecsBinary)
        {
            SecsBinary ppbody = (SecsBinary) replyData
