package EqpLogAlarmProcessStateIntoCSV_Common
...
''')
class EqpLogAlarmProcessStateIntoCSV_Common_1
{
    ...
    public void execute()
    {
        ...
        def alarmId = 0, alarmName = "Not Defined", processId = 0, processName = "Not Defined"
        alarmId = secsAlarm.getId()
        alarmName = secsAlarm.getName()
        if(alarmName.length() == 0)
        {
            alarmName = secsAlarm.getText()
        }
        ...
        if(!portList.empty)
        {
            for(port in portList)
            {
                ...
                if(secsAlarm.getName().contains(portId))
                {
        ...
        logger.info("Jimmy Log Info: '$alarmId, $alarmName, $processId, $processName'")
        ...
        if(file.exists())
        {
            if(file.length() == 0)
            {
                ...
                writer.println("$timestamp,,,$alarmId,$alarmName,$processId,$processName")
            }
            else
            {
                ...
                writer.print("$timestamp,,,$alarmId,$alarmName,$processId,$processName")
            }
        }
