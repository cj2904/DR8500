package ModbusHandleMultipleWipData
...
''')
class ModbusHandleMultipleWipData_1
{
    ...
    public void execute()
    {
        ...
        for (lt in lotList)
        {
            ...
            if (isTrackInEqp)
            {
                ...
                logger.info("Lot '" + lot.getId() + "' is running at chamber '" + chamberId + "@" + chamberEqpId + "@" + eqId + "'")
                ...
                if (wipDataMapping.size()>0)
                {
                    ...
                    for (wipDataItem in moveOutWipData)
                    {
                        ...
                        if (wipDataType==2)
                        {
                            pattern = TscConfig.getStringProperty("WipDataMultipleWaterRegExp","W_.*[0-9]")
                            wipDataStoreKey = lot.getEquipmentId() + "_LastWaterResistence"
                        }
                        else if (wipDataType==1)
                        {
