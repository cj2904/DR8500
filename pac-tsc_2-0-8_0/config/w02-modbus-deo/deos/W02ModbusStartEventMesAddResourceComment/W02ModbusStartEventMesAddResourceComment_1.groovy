package W02ModbusStartEventMesAddResourceComment
...
@CompileStatic
@Deo(description='''
modbus start event message and comment
''')
class W02ModbusStartEventMesAddResourceComment_1 {
	...
	public void execute() {
		...
		if(eqLogicalId.length() > 0) {
			...
			def request = new SetResourceCommentRequest(eqLogicalId, "[$status] Event 'Start' at equipment $eqLogicalId @ $eventTime")
