package EapQueryPortStatus_AOI
...
''')
class EapQueryPortStatus_AOI_1 {
	...
	public void execute() {
		...
		def eqpPort = "1"
		def eqpPortState = getSecsValueFromEqp("SV@Port01_TransferState")
		...
		for (var in portStatus) {
			if (portVal.equalsIgnoreCase(var)) {
				logger.info("The Equipment Port '$eqpPort' for equipment '$eqpId' with status code '$portVal' is ready to start ! ")
				portState = true
				//break
			}
		}
		if (!portState) {
			...
			throw new Exception("The Equipment Port '$eqpPort' for equipment '$eqpId' with status code '$portVal' is not ready to start ! ")
		}
	}
