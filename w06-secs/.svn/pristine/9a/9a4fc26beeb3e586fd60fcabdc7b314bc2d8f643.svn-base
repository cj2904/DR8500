package EapValidateRecipeParameter_ALN
...
''')
class EapValidateRecipeParameter_ALN_1 {
	...
	public Map<String,String> getEqpRecipeFromEqp(String eqpRecipe)
    {
        ...
        def recipeList = ((SecsComposite)dataList.get(3)).getValueList()
        logger.info("PPID=$param1, MDLN=$param2, SOFTREV=$param3")
		
        for (recipe in recipeList)     //  list of each module
        {
			...
			def value = ((SecsAsciiItem)valueList.get(0)).getString()
