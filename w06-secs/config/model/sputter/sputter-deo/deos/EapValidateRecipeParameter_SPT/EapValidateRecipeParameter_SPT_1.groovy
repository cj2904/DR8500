package EapValidateRecipeParameter_SPT
...
''')
class EapValidateRecipeParameter_SPT_1 {
	...
	public Map getValueFromEqp(String eqpRecipe) {
		...
		logger.info("This is a Print for RecipeBody :" + recipeBody)
		
		    recipeBody = recipeBody.replace("<", "").trim()
			recipeBody = recipeBody.replace(">", "").trim()
		
		def recipeBodySplit1 = recipeBody.split("\r\n")
		for (recipe in recipeBodySplit1) {
			def recipeBodySplit2 = recipe.split("=")
			def recipeTitle = recipeBodySplit2[0].trim()
			def newRecipeParam = recipeBodySplit2[1].trim()
			
			recipeMap.put(recipeTitle, newRecipeParam);
		}
